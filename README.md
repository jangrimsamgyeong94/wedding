...
개발 서버로 연 화면에서는 사진에 마우스를 올리면 오른쪽 위에 `×` 버튼이 나타납니다. 누르면 해당 사진 파일이 폴더에서도 즉시 삭제되고, 이미지 목록과 화면이 자동으로 갱신됩니다. 이 버튼은 로컬 개발 서버에서만 보이며 GitHub Pages에는 표시되지 않습니다.

여러 장을 지우려면 상단의 `여러 장 선택`을 누르고 사진들을 선택한 다음 `선택 삭제`를 누르세요. 선택된 실제 이미지 파일들이 한 번에 삭제되고 화면이 다시 만들어집니다.
